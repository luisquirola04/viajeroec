...
export default function EditarLugarPage({ params }) {
    ...
    // Cargar datos usando 'external' y arreglando las categorías
    useEffect(() => {
        ...
        const cargarDatos = async () => {
            try {
                ...
                // --- CARGAR CATEGORÍAS (Padres e Hijas) ---
                ...
                if (resLugar && resLugar.data) {
                    ...
                    // Parsear multimedia existente y tratarla como "enlaces" para no volver a subirla
                    if (d.Multimedia && d.Multimedia.length > 0) {
                        const imagenesExistentes = d.Multimedia.map(m => ({
                            type: 'link', // Lo tratamos como link porque ya es una URL alojada
                            content: m.url,
                            preview: m.url
                        }));
                        setMediaItems(imagenesExistentes);
                    }
    ...
    // --- MANEJO DE MULTIMEDIA (Igual que el Crear) ---
    const handleFileSelect = (e) => {
        const files = Array.from(e.target.files);
        if (!files.length) return;

        if (mediaItems.length + files.length > 4) {
            Swal.fire('Límite excedido', 'Solo puedes añadir un máximo de 4 elementos multimedia.', 'warning');
            return;
        }
    ...
    const handleAddLink = () => {
        if (mediaItems.length >= 4) return Swal.fire('Límite alcanzado', 'Ya has alcanzado el máximo de 4 elementos.', 'warning');
        if (!linkInput.trim()) return;
        
        try {
            new URL(linkInput);
            setMediaItems(prev => [...prev, { type: 'link', content: linkInput.trim(), preview: linkInput.trim() }]);
            setLinkInput(''); 
        } catch (_) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Subir archivos nuevos a Cloudinary y recolectar enlaces existentes
            for (const item of mediaItems) {
                if (item.type === 'link') {
                    urlsFinales.push(item.content); // URL existente en base de datos o Youtube
            ...
            const dataToSend = {
                ...
                externalLugar: external, // Enviamos el identificador del lugar
            ...
            if (res && res.code === 200) {
                Swal.fire({ icon: 'success', title: '¡Lugar Actualizado!', text: res.msj }).then(() => {
    ...
    if (loading) return <div className="min-h-screen flex items-center justify-center bg-slate-50 text-slate-500 font-bold">Cargando datos del lugar...</div>;
                ...
                <div className="bg-white w-full max-w-6xl rounded-2xl shadow-xl overflow-hidden flex flex-col md:flex-row border border-slate-100">
                    ...
                    <div className="md:w-1/3 bg-slate-50 p-6 border-r border-slate-100 flex flex-col">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <span className={`text-xs font-bold px-2 py-1 rounded ${mediaItems.length >= 4 ? 'bg-red-100 text-red-600' : 'bg-blue-100 text-blue-700'}`}>
                                {mediaItems.length}/4
                            </span>
                        </div>
                        ...
                        <label className={`border-2 border-dashed rounded-xl w-full h-24 flex flex-col items-center justify-center transition-all ${mediaItems.length >= 4 ? 'bg-gray-100 cursor-not-allowed opacity-60' : 'bg-white hover:bg-blue-50 cursor-pointer mb-4'}`}>
                            <span className="text-slate-500 font-medium">Seleccionar Fotos/Videos</span>
                            <input type="file" multiple className="hidden" accept="image/*,video/*" onChange={handleFileSelect} disabled={mediaItems.length >= 4} />
                        </label>
                    ...
                    <div className="md:w-2/3 p-8 overflow-y-auto max-h-[90vh]">
                        <h2 className="text-2xl font-bold text-slate-800 mb-6">2. Información y Ubicación (Editar)</h2>
                        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-5">

                            <div className="md:col-span-2">
                                <label className="label-text text-sm font-semibold text-slate-700">Nombre</label>
                                <input type="text" required className="w-full p-2 border border-slate-300 rounded-lg outline-none focus:border-blue-500" value={form.nombre} onChange={(e) => setForm({ ...form, nombre: e.target.value })} />
                            </div>

                            {/* --- SELECT DE CATEGORÍA CON OPTGROUPS --- */}
                            <div>
                                <label className="label-text text-sm font-semibold text-slate-700">Categoría</label>
                                <select required className="w-full p-2 border border-slate-300 rounded-lg outline-none focus:border-blue-500" value={form.categoria_external} onChange={(e) => setForm({ ...form, categoria_external: e.target.value })}>
                            ...
                            <div>
                                ...
                                <select required className="w-full p-2 border border-slate-300 rounded-lg outline-none focus:border-blue-500" value={form.parroquia_external} onChange={(e) => setForm({ ...form, parroquia_external: e.target.value })}>
                                    <option value="">-- Seleccionar --</option>
                            ...
                            <div className="md:col-span-2 bg-slate-50 p-4 rounded-xl border border-slate-200">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                    <div>
                                        ...
                                        <select className="w-full p-2 rounded-lg border border-slate-300 text-sm outline-none focus:border-blue-500" value={schedule.diaInicio} onChange={(e) => setSchedule({ ...schedule, diaInicio: e.target.value })}>
                                    ...
                                    <div>
                                        ...
                                        <select className="w-full p-2 rounded-lg border border-slate-300 text-sm outline-none focus:border-blue-500" value={schedule.diaFin} onChange={(e) => setSchedule({ ...schedule, diaFin: e.target.value })}>
                                ...
                                <div className="mt-3 text-center">
                                    ...
                                    <span className="text-sm font-medium text-blue-700 bg-blue-100 px-2 py-1 rounded">{form.horario || "Selecciona horario..."}</span>
                                </div>
                            </div>
                            ...
                            <div className="md:col-span-2">
                                ...
                                <div className="grid grid-cols-2 gap-4 mb-3">
                                    <div>
                                        ...
                                        <input type="text" className="w-full p-2 border border-slate-300 rounded-lg text-sm outline-none focus:border-blue-500 bg-white" value={form.latitud} onChange={(e) => setForm({ ...form, latitud: e.target.value })} />
                                    </div>
                                    <div>
                                        <span className="text-xs text-slate-400 font-bold uppercase">Longitud</span>
                                        <input type="text" className="w-full p-2 border border-slate-300 rounded-lg text-sm outline-none focus:border-blue-500 bg-white" value={form.longitud} onChange={(e) => setForm({ ...form, longitud: e.target.value })} />
                                    </div>
                                </div>
                            ...
                            <div className="md:col-span-2">
                                ...
                                <textarea required rows={3} className="w-full p-2 border border-slate-300 rounded-lg outline-none focus:border-blue-500 resize-none" value={form.descripcion} onChange={(e) => setForm({ ...form, descripcion: e.target.value })} />
                            </div>

                            <div className="md:col-span-2 pt-4">
                                <button type="submit" disabled={sending} className="w-full py-3 rounded-xl text-white font-bold bg-blue-600 hover:bg-blue-700 transition-all shadow-lg hover:shadow-blue-500/30 flex justify-center items-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed">
                                    {sending ? (
                                        <>
