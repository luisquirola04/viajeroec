...
import { getProvincia, editarProvincia } from "@/hooks/ServiceProvincia"; // Asegúrate de tener estos servicios
...
export default function EditarProvincia() {
  ...
  const [formData, setFormData] = useState({
    ...
    info: "",
    imagen: "",
    externalProvincia: external // Debe coincidir con el req.body del backend
  });

  useEffect(() => {
    if (external) cargarProvincia();
  }, [external]);

  const cargarProvincia = async () => {
    const token = sessionStorage.getItem("token");
    if (!token) {
        router.push("/admin/provincia");
        return;
    }

    try {
      const respuesta = await getProvincia(token, external);
      // Ajusta si tu backend lo devuelve como respuesta.provincia o respuesta.data
      const p = respuesta.provincia || respuesta; 
      ...
      if (p) {
        setFormData({
            ...
            info: p.info || "",
            imagen: p.imagen || "",
            externalProvincia: external as string
        });
      }
    } catch (error) {
      Swal.fire("Error", "No se pudo cargar la información de la provincia", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        const respuesta = await editarProvincia(token, formData);
        if (respuesta && respuesta.code === 200) {
            await Swal.fire('¡Éxito!', 'Provincia actualizada correctamente.', 'success');
    ...
    <div className="flex min-h-screen bg-slate-50">
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto h-screen flex justify-center">
        <div className="w-full max-w-3xl">
            <Link href="/admin/provincia" className="text-teal-600 hover:text-teal-800 flex items-center gap-2 mb-6 font-medium">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
                Volver
            </Link>

            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
                <h1 className="text-2xl font-bold text-slate-800 mb-6">Editar Provincia</h1>
                ...
                {loading ? (
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div>
                            <label className="block text-sm font-semibold text-slate-700 mb-2">Nombre de la Provincia</label>
                        ...
                        <div>
                            ...
                            <button type="button" onClick={() => fileInputRef.current?.click()} className="w-full bg-slate-100 hover:bg-slate-200 text-slate-700 font-medium py-3 rounded-xl border border-slate-300 transition-colors flex justify-center items-center gap-2">
